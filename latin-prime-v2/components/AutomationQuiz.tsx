...
export default function AutomationQuiz() {
  ...
  function submitLead() {
    ...
    const selectedPlan = getPlan(pain, volume);
    const selectedScore = getScore(pain, volume);
    const selectedAutomations = (AUTO_MAP[pain ?? ""] ?? AUTO_MAP.slow_followup).map(a => a.t);
    const industryItems = detailIndustry ? (INDUSTRY_DETAILS[detailIndustry]?.items ?? []) : [];
    ...
    const leadData = {
      ...
      score: selectedScore,
      score_label: getScoreSub(selectedScore),
